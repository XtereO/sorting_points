...
from utils import shifted_point
...
def pick_next_point_rotated_atan2(points: List[List[float]], indexed_points: List[Tuple], indexed_pivot_point: Tuple[float]) -> int:
    ...
    shifted_pivot_x, shifted_pivot_y = shifted_point(
        indexed_points, indexed_pivot_point)
    ...
    for i, p in indexed_points:
        ...
        indexed_angles.append({"angle": angle, "index": i})

    indexed_angles = [ip for ip in indexed_angles if ip["angle"] >= -1.5708]
...
def pick_next_point_shifted_scalar_product(points: List[List[float]], indexed_points: List[Tuple], indexed_pivot_point: Tuple[float]) -> int:
    ...
    indexed_angles = []
    for i, p in indexed_points:
        sign = 1 if -shifted_pivot_y * \
            (p[0]-x_mean) + shifted_pivot_x*(p[1]-y_mean) > 0 else -1
    ...
    min_indexed_angle = min(indexed_angles, key=lambda ia: (
...
def pick_next_point_Jarvis(indexed_points: List[Tuple], pivot_point: List[float], prev_point: List[float]) -> int:
    current_vector = [pivot_point[0] - prev_point[0], 
                      pivot_point[1] - prev_point[1]]

    indexed_points_props = []
    for i, p in indexed_points:
        new_vector = [p[0] - pivot_point[0], p[1] - pivot_point[1]]
        len_nv = (new_vector[0]**2 + new_vector[1]**2)**0.5
        cross = -(new_vector[0]*current_vector[1] -
                  new_vector[1]*current_vector[0])
        dot = (new_vector[0]*current_vector[0] +
               new_vector[1]*current_vector[1])
        angle = atan2(cross, dot)
        indexed_points_props.append(
            {"angle": angle, "len_nv": len_nv, "z": p[2], "index": i})

    max_indexed_cos = max(indexed_points_props, key=lambda ip: (
        ip["angle"], -ip["len_nv"], -ip["z"]))
